...
class Model(object):
		...
		for i in range(1):
			for j in range(self.batch_size):
				...
				for k in range(n): 
					...
					#p = pred[j][k][self.dull[i][k+1]]
					p = tf.cond(p < 1e-20, lambda: tf.constant(1e-20), lambda: p)
					total_p += tf.log(p)				
				sum_p.append(total_p/float(n))
		
		for i in [self.EOS, self.Unknown, 21]:
			for j in range(self.batch_size):
				p = pred[j, :self.num_steps-1, i]
				for k in range(self.num_steps-1):
					tf.cond(p[k] < 1e-30, lambda: tf.constant(1e-30), lambda: p[k])
				p = tf.reduce_mean(tf.log(p))
				sum_p.append(p)
				#break
		#temp = []
		#for i in range(len(sum_p)):
		#	if not math.isinf(sum_p[i]): temp.append(sum_p)
		#sum_p = [p in sum_p if p != (-inf)]
		#sum_p = temp
		return -np.sum(sum_p)/len(sum_p)#/len(sum_p)'''
	...
	def initialize_model(self):
		...
		if self.first_train == 0:
			self.saver.restore(self.sess, self.path+"model" + name + ".ckpt")
	...
	def cut_batch_test(self):
		self.xx_batch = self.x_test_vocab
		
	def testing(self):
		self.cut_batch_test()
		pred = self.sess.run(self.pred_test, feed_dict = {
			self.x_nn: self.xx_batch, self.bs: self.x_test.shape[0], self.keep_prob: self.drop})
